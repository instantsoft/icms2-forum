...
<?php if (!empty($thread['description'])) { ?>
    <p class="<?php if (!empty($thread['badges'])) { ?><?php } else { ?>mt-n3<?php } ?>">
...
<?php
$this->renderChild($tpl_posts, array(
    'posts'         => $posts,
    'num'           => $num,
    'thread'        => $thread,
    ...
    <div class="row">
        <?php if (!empty($thread['prev_thread'])) { ?>
            <div class="col-md mb-2 mb-md-0">
                <a class="btn btn-secondary btn-block" href="<?php echo href_to('forum', $thread['prev_thread']['slug'] . '.html'); ?>" title="<?php echo LANG_FORUM_PREVIOUS_THREAD; ?>">
        ...
        <?php if (!empty($thread['next_thread'])) { ?>
            <div class="col-md">
                <a class="btn btn-secondary active btn-block" href="<?php echo href_to('forum', $thread['next_thread']['slug'] . '.html'); ?>" title="<?php echo LANG_FORUM_NEXT_THREAD; ?>">
...
<?php if (!$thread['is_closed']) { ?>
    ...
    <div class="thread-fast-edit mt-3 mt-md-4">
        ...
        <?php if ($options['fast_answer'] && $thread_access->is_can_write) { ?>

            <div class="h4">
                <?php html_svg_icon('solid', 'reply-all'); ?>
                <?php echo LANG_FORUM_FAST_ANSWER; ?>
            </div>
        ...
        <?php } elseif ($thread_access->is_can_write) { ?>
            ...
            <div class="thread-fast-edit-header">
                <a class="add_post btn btn-outline-primary" href="<?php echo href_to('forum', 'post_add', $thread['id']); ?>">
                    <?php echo LANG_FORUM_NEW_MESSAGE; ?>
                </a>
            </div>
